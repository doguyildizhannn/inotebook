...
const AddNote = () => {
    ...
    const addNoteClicked = async (e) => {
        e.preventDefault();
        let success = await addNote(note.title, note.description, note.tag);
        if (success) setNote({ title: "", description: "", tag: "" });
    }
    const onChange = (e) => {
    ...
    return (
        <div className="container my-3">
            ...
            <form className='my-3'>
                ...
                <button type="submit" className="btn btn-primary" onClick={addNoteClicked}>Add Note</button>
            </form>
        </div>
    )
}
